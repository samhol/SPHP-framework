...
class LabellerTest extends TestCase {
  ...
  public function testUseInTable(Labeller $labeller) {
    ...
    foreach ($table->tbody()->getComponentsByObjectType(\Sphp\Html\Tables\Tr::class) as $tr) {
      $this->assertSame('first', $tr->getCell(0)->getAttribute('data-label'));
      $this->assertSame('second', $tr->getCell(1)->getAttribute('data-label'));
      $this->assertSame('third', $tr->getCell(2)->getAttribute('data-label'));
      $this->assertSame('fourth', $tr->getCell(3)->getAttribute('data-label'));
    }
  }
