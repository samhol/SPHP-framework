...
class CsvTableBuilderTest extends TestCase {
  ...
  protected function checkHeading(CsvFile $csvData, Table $table, int $linenumbers): void {
    ...
    foreach ($headings as $key => $value) {
      $this->assertEquals((string) $value, (string) $tr->getCell($key)->contentToString());
    }
  }
  ...
  public function buildParams(): array {
    ...
    $data[] = [false, LineNumberer::LEFT, 2, 20];
    return $data;
  }
  ...
  public function testBuild(bool $useHead, int $linenumbers, int $from, int $to): void {
    ...
    $this->assertSame($builder, $builder->buildFromFile('./sphp/php/tests/files/10.csv'));
    $table = $builder->build();
