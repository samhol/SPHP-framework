...
public function testConstructor() :void{
    ...
    $this->assertNull($attrs->getStringValue('data-attr'));
}
  /**
  ...
  public function testProtectingClassValues(): void {
    $attrs = new AttributeContainer();
  ...
  public function testSetImmutableInstance(): AttributeContainer {
    $attrs = new AttributeContainer();
    $attrs->setAttribute('a', 'b');
  ...
  /**
   * @depends testSetImmutableInstance
   * 
   * @param AttributeContainer $attrs
   * @return void
   */
  public function testRemoveImmutableAttributeThrowsException(AttributeContainer $attrs): void {
    $this->expectException(ImmutableAttributeException::class);
    $attrs->remove('a');
  }
   ...
   * @return void
   */
  public function testSetImmutableAttributeThrowsException(AttributeContainer $attrs): void {
    $this->expectException(ImmutableAttributeException::class);
    $attrs->setAttribute('a', 'b');
  }
   ...
   * @return void
   */
  public function testProtectImmutableAttributeThrowsException(AttributeContainer $attrs): void {
    $this->expectException(ImmutableAttributeException::class);
    $attrs->protect('a', 'b');
  }
  ...
  public function testGetters(): void {
    ...
    $this->assertInstanceof(MapAttribute::class, $styles = $attrs->getAttribute('style'));
    ...
    $this->assertInstanceof(MapAttribute::class, $attrs1->styles());
  }
   ...
   * @return void
   */
  public function testGetStringValue($value, ?string $output): void {
    $attrs = new AttributeContainer();
    $attrs->setAttribute('data-attr', $value);
    $this->assertSame($output, $attrs->getStringValue('data-attr'));
  }
   ...
   */
  public function testScalarAttributeSetting($value) {
    $name = 'data-attr';
    $attrs = new AttributeContainer();
    $attrs->setAttribute($name, $value);
    $this->assertInstanceof(ScalarAttribute::class, $attr = $attrs->getAttribute($name));
    $this->assertSame($value, $attrs->getValue($name));
    $attr->setValue('foo');
    $this->assertSame('foo', $attrs->getValue($name));
    $this->assertFalse($attrs->isProtected($name));
  }
  ...
  public function objectData(): array {
    return [
        [new MapAttribute("data-bar", \Sphp\Html\Attributes\PropertyParser::singelton(':', ','))],
        [new \Sphp\Html\Attributes\StyleAttribute('style')],
  ...
  public function testLockDemand($name, $value) {
    $this->assertFalse($this->attrs->isAlwaysVisible($name));
    $this->attrs->forceVisibility($name);
    $this->assertTrue($this->attrs->isAlwaysVisible($name));
    $this->attrs->setAttribute($name, $value);
  ...
  public function testNotExists($attrName) {
    ...
    $this->assertTrue($this->attrs->isAlwaysVisible($attrName) === false);
  ...
  public function testStyleSettings($data) {
    $attrs = new AttributeContainer();
    $this->assertInstanceOf(MapAttribute::class, $attrs->styles()->setValue($data));
  }
