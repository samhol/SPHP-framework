...
class FinnishCalendar extends Calendar {
  ...
  public static function create(int $year, int $month = null): Calendar {
    ...
    $calendar->insertAnnualHoliday("$year-4-27", "National War Veterans' Day")->setFlagDay(true);
    $calendar->insertAnnualHoliday("$year-5-1", 'May Day')->setFlagDay(true)->setNationalHoliday();
    $calendar->insertAnnualHoliday("$year-5-9", 'Europe Day')->setFlagDay(true);
    $calendar->insertAnnualHoliday("May $year second sunday", "Mothers's Day")->setFlagDay(true);
    $calendar->insertAnnualHoliday("May $year third sunday", 'memorial day')->setFlagDay(true);
    ...
    $calendar->insertAnnualHoliday($j, "Midsummer's Eve")->setNationalHoliday();
    $calendar->setBirthDay("$year-7-6", 'Eino Leino')->setFlagDay(true);
    $calendar->setBirthDay("$year-10-10", 'Aleksis Kivi')->setFlagDay(true);
    $calendar->insertAnnualHoliday("$year-10-10", 'Day of Finnish literature');
    $calendar->insertAnnualHoliday(static::getAllSaintsDay($year), 'All Saints Day')->setNationalHoliday();
    $calendar->insertAnnualHoliday("November $year second sunday", "Father's Day")->setFlagDay(true);
