...
class Calendar extends BaseCalendar {
  ...
  public function createYear(int $year) {
    $this->insertAnnualHoliday("$year-1-1", "New Year's Day")->setNationalHoliday();
    $this->insertAnnualHoliday("$year-1-6", 'Epiphany')->setNationalHoliday();
    $this->setBirthDay("$year-2-5", 'Johan Ludvig Runeberg')->setFlagDay(true);
    $this->insertAnnualHoliday("$year-2-28", "Day of Kalevala")->setFlagDay(true);
    //$this->mergeCalendar();
    $this->insertAnnualHoliday("$year-4-27", "National War Veterans' Day")->setFlagDay(true);
    $this->insertAnnualHoliday("$year-5-1", 'May Day')->setFlagDay(true)->setNationalHoliday();
    $this->insertAnnualHoliday("$year-5-9", 'Europe Day')->setFlagDay(true);
    $this->insertAnnualHoliday("May $year second sunday", "Mothers's Day")->setFlagDay(true);
    $this->insertAnnualHoliday("May $year third sunday", 'memorial day')->setFlagDay(true);
    $j = Date::fromString("$year-6-20")->modify('next Saturday');
    $this->setBirthDay("$year-6-4", 'Carl Gustaf Emil Mannerheim')->setFlagDay(true);
    $this->insertAnnualHoliday($j, "Midsummer's Eve")->setNationalHoliday();
    $this->setBirthDay("$year-7-6", 'Eino Leino')->setFlagDay(true);
    $this->setBirthDay("$year-10-10", 'Aleksis Kivi')->setFlagDay(true);
    $this->insertAnnualHoliday("$year-10-10", 'Day of Finnish literature');
    $this->insertAnnualHoliday(static::getAllSaintsDay($year), 'All Saints Day')->setNationalHoliday();
    $this->insertAnnualHoliday("November $year second sunday", "Father's Day")->setFlagDay(true);
    $this->insertAnnualHoliday("$year-12-6", 'Independence Day')->setFlagDay(true)->setNationalHoliday();
    $this->setBirthDay("$year-12-8", 'Jean Sibelius')->setFlagDay(true);
    $this->insertAnnualHoliday("$year-12-24", 'Christmas Eve');
    $this->insertAnnualHoliday("$year-12-25", 'Christmas Day')->setNationalHoliday();
    $this->insertAnnualHoliday("$year-12-26", 'Boxing Day')->setNationalHoliday();
    return $this;
  }
