...
use Sphp\Apps\Contact\Views\FormView;
use Sphp\Apps\Contact\Views\ResultView;
...
class Controller {
  ...
  public function getContactData(): DataObject {
    ...
    if (!$this->validator->isValid($post)) {
      ...
      $this->data->errors = $this->validator->errors()->toArray();
    }
  ...
  public function processFormSubmission(): void {
    ...
    $domain = $url->getHost();
    $sheme = $url->getScheme();
    (new GenericHeader('Location', "$sheme://$domain/yhteystiedot/yhteydenotto#vastaus"))->save();
  }
