...
//const PHONENUMBER_PATTERN = "/^((\+\d{4,6})|(\+\d{3}[\ ]?\d{1,3})){1}|(\d{2,4}){1}([\ ]?)(\d{3}[\ ]?\d{3,4})$/";
  const PHONENUMBER_PATTERN = "/^\+?\d{2}[\d\ -]{3,14}$/";
  const FI_ZIPCODE_PATTERN = "/^\d{5}$/";
  const SWE_ZIPCODE_PATTERN = "/^(s-|S-){0,1}[0-9]{3}\s?[0-9]{2}$/";
  const US_ZIPCODE_PATTERN = "/^\d{5}(\-\d{4})?$/";
  const UK_ZIPCODE_PATTERN = "/^(([Gg][Ii][Rr] 0[Aa]{2})|((([A-Za-z][0-9]{1,2})|(([A-Za-z][A-Ha-hJ-Yj-y][0-9]{1,2})|(([A-Za-z][0-9][A-Za-z])|([A-Za-z][A-Ha-hJ-Yj-y][0-9]?[A-Za-z])))) [0-9][A-Za-z]{2}))$/ui";
  const TIME_PATTERN = "/^(([0-9])|([0-1][0-9])|([2][0-3]))[:.,;](([0-9])|([0-5][0-9]))$/";
  const YEAR_PATTERN = "/^(19|20)([0-9]{1,2})$/"; //
  const FI_DATE_PATTERN = "/^([0-9]{1,2})[\.]([0-9]{1,2})[\.](19|20)([0-9]{1,2})$/";
